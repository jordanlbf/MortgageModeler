    ...
    <>
      ...
      >
        {COLUMNS.map((h, i) => (
          <div
            key={h}
            className={`text-[10px] font-semibold uppercase tracking-widest text-foreground/25 ${
      ...
      <div className="custom-scrollbar overflow-y-auto" style={{ maxHeight: height }}>
        {rows.map((row, ri) => (
          ...
          >
            <div className="text-sm text-foreground/25">{row.period}</div>
            <div className="text-right text-foreground/35">{formatCurrency(row.opening_balance)}</div>
            <div className="text-right text-foreground/60">{formatCurrency(row.scheduled_repayment)}</div>
            ...
            <div className="text-right text-foreground/35">{formatCurrency(row.closing_balance)}</div>
          </div>
        ))}
      </div>
    </>
