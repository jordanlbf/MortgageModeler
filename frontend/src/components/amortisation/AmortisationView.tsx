...
function ChartTooltip({
  ...
  return (
    <div className="rounded-xl border border-white/[0.06] bg-[#0a0a14] px-5 py-4 shadow-lg shadow-black/40">
      <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/25">
        Year {label}
      </div>
      {payload.map((entry, i) => (
...
export default function AmortisationView() {
  ...
  return (
    <>
      {/* ── Header ── */}
      <header className="flex items-center justify-between border-b border-white/[0.06] px-9 py-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[17px] font-semibold tracking-tight text-white/75">MortgageModeler</span>
        </div>
        <div className="flex rounded-lg border border-white/[0.06] bg-[#08080e] p-[3px]">
      ...
      <div className="px-9 py-5">
        ...
        <div className="mb-5 grid grid-cols-3 gap-5">
          <GlassCard className="flex flex-col items-center justify-center py-7 text-center">
            ...
            <div className="mt-2 text-[12px] font-medium tracking-wide text-white/40">
          ...
          <GlassCard className="flex flex-col items-center justify-center py-7 text-center">
            ...
            <div className="mt-2 text-[12px] font-medium tabular-nums tracking-wide text-white/40">
          ...
          <GlassCard className="flex flex-col items-center justify-center py-7 text-center">
            ...
            <div className="mt-2.5 text-[28px] font-normal leading-none tabular-nums text-white/75">
        ...
        <div className="mb-5 grid grid-cols-[4fr_1fr] gap-5">
          <GlassCard>
            ...
            <div className="grid grid-cols-4">
              ...
              <div className="border-l border-white/[0.04] px-5 py-4">
        ...
        <GlassCard className="overflow-hidden">
          <div className="relative flex items-center justify-between border-b border-white/[0.04] px-5 py-3.5">
            {/* Legend (left) */}
                ...
                <ResponsiveContainer width="100%" height={fillHeight}>
                  <AreaChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 20 }}>
                    ...
                    <XAxis
                      dataKey="y"
                      stroke="transparent"
                      tick={{ fill: "rgba(255,255,255,0.25)", fontSize: 11, fontWeight: 500, dy: 12 }}
                      tickLine={false}
          ...
          {view === "table" && (
            <>
              <div className="grid grid-cols-[52px_1fr_1fr_1fr_1fr_1fr] border-b border-white/[0.04] px-5 py-2.5">
              ...
              <div className="custom-scrollbar overflow-y-auto" style={{ maxHeight: fillHeight }}>
                {tableRows.map((row, ri) => (
                  <div
                    key={row.period}
                    className={`grid grid-cols-[52px_1fr_1fr_1fr_1fr_1fr] px-5 py-2.5 text-[15px] tabular-nums transition-colors hover:bg-white/[0.02] ${
                      ri < tableRows.length - 1 ? "border-b border-white/[0.04]" : ""
                    }`}
                  >
                    <div className="text-sm text-white/25">{row.period}</div>
                    <div className="text-right text-white/40">{formatCurrency(row.opening_balance)}</div>
                    <div className="text-right text-white/75">{formatCurrency(row.scheduled_repayment)}</div>
                    <div className="text-right text-pink-400/85">{formatCurrency(row.interest)}</div>
                    <div className="text-right text-indigo-400/85">{formatCurrency(row.principal_paid)}</div>
                    <div className="text-right text-white/40">{formatCurrency(row.closing_balance)}</div>
                  </div>
                ))}
              </div>
            </>
