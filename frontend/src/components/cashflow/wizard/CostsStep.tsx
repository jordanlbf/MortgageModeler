    ...
    <div className="flex flex-col gap-7">
      <div className="flex gap-3.5">
        <div className="flex flex-col gap-2 flex-1">
          ...
          <Input value={s.councilRates} {...currencyInput(s.setCouncilRates)} />
        </div>
        <div className="flex flex-col gap-2 flex-1">
          <label className="text-xs font-medium text-fg-secondary">Water rates (p.a.)</label>
          <Input value={s.waterRates} {...currencyInput(s.setWaterRates)} />
        </div>
      </div>
      <div className="flex gap-3.5">
        <div className="flex flex-col gap-2 flex-1">
          <label className="text-xs font-medium text-fg-secondary">Insurance (p.a.)</label>
          <Input value={s.insurance} {...currencyInput(s.setInsurance)} />
        </div>
        <div className="flex flex-col gap-2 flex-1">
          <label className="text-xs font-medium text-fg-secondary">Maintenance (%)</label>
          <Input value={s.maintenance} onChange={(e) => s.setMaintenance(e.target.value)} />
        </div>
      </div>
      ...
      {s.hasStrata && (
        <div className="flex flex-col gap-2">
          ...
          <Input value={s.strataFees} {...currencyInput(s.setStrataFees)} />
        </div>
      )}
    </div>
  );
