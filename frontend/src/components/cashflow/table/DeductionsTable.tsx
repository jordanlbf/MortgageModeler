  ...
  return (
    <>
      ...
      {panel === "unified" && (() => {
        ...
        const totalColor = isInvestment ? "#a78bfa" : "var(--color-negative)";
        ...
        <table className="border-collapse tabular-nums text-[13px] leading-[1.4] whitespace-nowrap w-full min-w-full table-auto">
          <thead>
            <tr className="h-7 border-b-0" style={{ background: "var(--color-surface-raised)" }}>
              ...
              <th
                className="h-7 box-border pl-6 pr-3 text-center align-bottom text-[10px] font-semibold tracking-[0.06em] uppercase relative before:content-[''] before:absolute before:top-0 before:left-2 before:right-2 before:h-[3px] before:rounded-b-sm before:bg-current before:opacity-75 text-accent"
          ...
          <tbody>
            {yearData.map((y, i) => {
              ...
              return (
                <tr key={y.year} className={`${getRowClass(y.year, isMilestone)} ${isCollapsed ? "h-14" : ""}`} {...getRowHandlers(y.year, isMilestone)}>
                  ...
                  {holdExpanded && isInvestment && <td className="h-[52px] box-border px-3 text-right align-middle border-b border-b-white/[0.04] text-negative animate-[col-fade-in_0.25s_cubic-bezier(0.16,1,0.3,1)]">{formatDollarsSigned(Math.round(y.interestPortion))}</td>}
                  {holdExpanded && <td className="h-[52px] box-border px-3 text-right align-middle border-b border-b-white/[0.04] text-negative animate-[col-fade-in_0.25s_cubic-bezier(0.16,1,0.3,1)]">{formatDollarsSigned(Math.round(y.councilRates + y.waterRates))}</td>}
                  {holdExpanded && <td className="h-[52px] box-border px-3 text-right align-middle border-b border-b-white/[0.04] text-negative animate-[col-fade-in_0.25s_cubic-bezier(0.16,1,0.3,1)]">{formatDollarsSigned(Math.round(y.insurance))}</td>}
                  {holdExpanded && <td className="h-[52px] box-border px-3 text-right align-middle border-b border-b-white/[0.04] text-negative animate-[col-fade-in_0.25s_cubic-bezier(0.16,1,0.3,1)]">{formatDollarsSigned(Math.round(y.maintenance))}</td>}
                  ...
                  {isInvestment && depExpanded && <td className="h-[52px] box-border pl-6 pr-3 text-right align-middle border-b border-b-white/[0.04] animate-[col-fade-in_0.25s_cubic-bezier(0.16,1,0.3,1)]" style={{ color: depColor, opacity: 0.7 }}>{formatDollarsSigned(Math.round(y.depDiv43))}</td>}
                  {isInvestment && depExpanded && <td className="h-[52px] box-border px-3 text-right align-middle border-b border-b-white/[0.04] animate-[col-fade-in_0.25s_cubic-bezier(0.16,1,0.3,1)]" style={{ color: depColor, opacity: 0.7 }}>{formatDollarsSigned(Math.round(y.depDiv40))}</td>}
            ...
            <tr className="row-summary">
              ...
              {holdExpanded && isInvestment && (
                <td className="box-border px-3 text-right align-middle text-negative font-medium text-[15px]" style={summaryBorder}>{formatDollarsSigned(Math.round(totals.interest))}</td>
              )}
              {holdExpanded && (
                <td className="box-border px-3 text-right align-middle text-negative font-medium text-[15px]" style={summaryBorder}>{formatDollarsSigned(Math.round(totals.rates))}</td>
              )}
              {holdExpanded && (
                <td className="box-border px-3 text-right align-middle text-negative font-medium text-[15px]" style={summaryBorder}>{formatDollarsSigned(Math.round(totals.insurance))}</td>
              )}
              {holdExpanded && (
                <td className="box-border px-3 text-right align-middle text-negative font-medium text-[15px]" style={summaryBorder}>{formatDollarsSigned(Math.round(totals.maint))}</td>
              )}
              <td className="box-border px-3 text-right align-middle text-[15px]" style={summaryBorder}>
                <span className="font-semibold text-negative">{formatDollarsSigned(Math.round(totals.holding))}</span>
              </td>
