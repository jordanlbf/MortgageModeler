...
def _build(**overrides):
    ...
    return build_ongoing_cost_projection(**args)
...
class TestYearOneCosts:
    ...
    def test_maintenance_cost_year_one(self):
        ...
        result = _build(purchase_price=500_000, maintenance_rate=0.005, annual_growth_rate=0.03)
    ...
    def test_management_fee_year_one(self):
        ...
        result = _build(weekly_rent=500, vacancy_weeks=2, management_rate=0.08, is_investment=True)
    ...
    def test_rental_income_year_one(self):
        result = _build(weekly_rent=600, vacancy_weeks=3)
        expected = 600 * (52 - 3)
...
class TestGrowthOverTime:
    ...
    def test_costs_grow_with_cost_growth_rate(self):
        ...
        result = _build(council_rates=2_000, annual_cost_growth_rate=0.05, projection_years=2)
    ...
    def test_water_rates_grow(self):
        result = _build(water_rates=1_000, annual_cost_growth_rate=0.03, projection_years=3)
        assert result.annual_costs[2].water_rates == pytest.approx(1_000 * 1.03 ** 2)

    def test_property_value_grows(self):
        result = _build(purchase_price=500_000, annual_growth_rate=0.04, projection_years=5)
        assert result.annual_costs[4].property_value == pytest.approx(500_000 * 1.04 ** 4)

    def test_rental_income_grows(self):
        result = _build(weekly_rent=500, vacancy_weeks=2, annual_rent_growth_rate=0.03, projection_years=3)
        y1_income = 500 * 50
...
class TestZeroGrowth:
    ...
    def test_zero_cost_growth(self):
        result = _build(annual_cost_growth_rate=0.0, annual_growth_rate=0.0,
                        annual_rent_growth_rate=0.0, projection_years=5)
        y1 = result.annual_costs[0]
    ...
    def test_zero_growth_property_value_flat(self):
        result = _build(annual_growth_rate=0.0, projection_years=5)
        for yc in result.annual_costs:
